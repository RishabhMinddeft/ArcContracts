...
contract RedeemCoupon is ERC721, ERC721URIStorage, Ownable, EIP712 {
    ...
    bytes32 messageHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", getMessageHash(_name,_address,_uri,_couponCode,_rewardCost)));
    return ecrecover(messageHash, v, 
    r, s);
}

    function getMessageHash(string memory _name, address _address, string memory _uri, uint256 _couponCode, uint256 _rewardCost) public pure returns (bytes32) {
	...
	{   (bytes32 r, bytes32 s, uint8 v) = extractRSV(voucher.signature);
        require(couponCreator[verifySignature(voucher.name,voucher.creator,voucher.uri,voucher.couponCode,voucher.rewardCost, v,r,s)], "Wrong signature.");
        // require(couponCodes[voucher.creator].length!=0,"code dosent exist");
        // require(verifyCouponCode(voucher.creator,voucher.couponCode));
    ...
    function safeMint(Coupon calldata voucher)
        public returns(bool)
    {
        // require(couponCreator[recover(voucher.signature,voucher.name,voucher.uri,voucher.creator)], "Wrong signature.");
        // require(msg.value >= voucher.rewardCost, "Not enough ether sent.");
