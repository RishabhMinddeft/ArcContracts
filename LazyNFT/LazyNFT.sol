...
contract LazyNFT is ERC721, ERC721URIStorage, Ownable, EIP712 {
    ...
    mapping(address=>bool) public nftCreator;
    event NFTMinted(address user, address redeemedBy, bytes signature );
    ...
    function approveCreator(address _creator) onlyOwner public returns (bool)  {
            nftCreator[_creator] = true;
            return true;
    }

    function mint(Coupon calldata voucher) public returns(bool)
	{   
        require(nftCreator[recover(voucher.signature,voucher.name,voucher.uri,voucher.creator,voucher.price)], "Wrong signature.");
		/*  Check if rewardCost is not 0 */
        require(voucher.price != 0,"reward cost is zero");
        require(voucher.redeemedBy != address(0),"zero address");
        safeMint(voucher);
        nftcount++;

		/* Trigger Event */
		emit NFTMinted(msg.sender, voucher.redeemedBy, voucher.signature);
  ...
  function recover(
    ...
    // uint chainId = 97; 
   (bytes32 r, bytes32 s, uint8 v) = extractRSV(signature);
    bytes32 hashStruct = keccak256(
      abi.encode(
          keccak256("set(string name,address creator,string uri,uint256 price)"),
