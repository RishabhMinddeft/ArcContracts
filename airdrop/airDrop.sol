/**
 *Submitted for verification at testnet.bscscan.com on 2023-04-28
 */
...
contract AirDrop is ERC721, Ownable {
    ...
    uint public constant publicMintFees = 100000000000000;
    uint public constant whiteListMintFees = 100000000000;

    constructor() ERC721('MindCoin', 'MDC') {
        _setOwner(msg.sender);
    }
    function airDrop(address[] memory _address) public onlyOwner {
        for (uint i = 0; i < _address.length; i++) {
            if (tokenCount[_address[i]] < 5) {
                _mint(_address[i], mintCounter.current());
                mintCounter.increment();
                tokenCount[_address[i]]++;
            }
        }
    }
    ...
    function Mint() public payable {
        require(
            tokenCount[msg.sender] < 5,
            'You have exceed maximum token amount in your wallet'
        );
        if (isWhiteListed[msg.sender]) {
            require(
                msg.value == whiteListMintFees,
                string(
                    abi.encodePacked(
                        'Please pay ',
                        Strings.toString(whiteListMintFees),
                        ' wei'
                    )
                )
            );
            ...
            require(
                msg.value == publicMintFees,
                string(
                    abi.encodePacked(
                        'Please pay ',
                        Strings.toString(publicMintFees),
                        ' wei'
                    )
                )
            );
        }
        _safeMint(msg.sender, mintCounter.current());
        mintCounter.increment();
